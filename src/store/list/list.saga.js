...
import { addNewListDocument, createUserDocumentFromAuth, deleteListDocument, getCollectionItem, getCollectionList, updateListDocument } from "../../utils/firebase/firebase.utils";
import { fetchListSuccess, fetchListFailed, setSearchString, setFilterSort, setFilterType, addShowListFailed, addShowListSuccess, deleteShowListFailed, deleteShowListSuccess, updateShowListFailed, updateShowListSuccess } from "./list.action";
import { LIST_ACTION_TYPES } from "./list.types";
import { getCurrentUser } from "../../utils/firebase/firebase.utils";


export function* getCurrentUserSnapshot(userAuth) {
  try {
...
export function* deleteShow({ payload: { id } }) {
  try {
    ...
    const deletedShow = yield call(getCollectionItem, userAuth, deletedShowRef);
...
export function* updateShow({ payload: formFields }) {
  try {
    const userAuth = yield call(getCurrentUser);
    const updatedShowRef = yield call(updateListDocument, userAuth, formFields);
    const updatedShow = yield call(getCollectionItem, userAuth, updatedShowRef);
